...
function Sprite(area,src,crop) {
    ...
    this.draw = function(){
        if (typeof crop !== 'undefined') {
            canvas.drawImage(this.image,this.area.x,this.area.y,this.area.w,this.area.h,this.crop.x,this.crop.y,this.crop.w,this.crop.h);
        } else {
            canvas.drawImage(this.image,this.area.x,this.area.y,this.area.w,this.area.h);
        }
...
function initiate() {
    ...
    */
    ...
    // TODO: design more puzzles (less overwhelming starters)
    
    // TODO: puzzle map with branches and required puzzles and optional puzzles
...
function Gem(type,point) {
    ...
    this.type = type;
    this.shake = 0;
    this.sprite = new Sprite(new Area2D((this.psudo.x*SCALE)+(SCALE/2),(this.psudo.y*SCALE)+(SCALE/2),SCALE,SCALE),'http://fritillo.com/demo/gemjam/img/gem/'+this.type+'.png');
    ...
    this.draw = function() {
        this.sprite.area = new Area2D(
                (this.psudo.x*SCALE)+(SCALE/2)+(-(Math.random(1)*this.shake)+(Math.random(2)*this.shake)),
                (this.psudo.y*SCALE)+(SCALE/2)+(-(Math.random(1)*this.shake)+(Math.random(2)*this.shake)),
                SCALE
                ,SCALE);
        switch (this.type) {
...
function Zone(type,point) {
    ...
    this.type = type;
    this.solved = false;
    this.sprite = new Sprite(new Area2D((this.point.x*SCALE)+(SCALE/2),(this.point.y*SCALE)+(SCALE/2),SCALE,SCALE),'http://fritillo.com/demo/gemjam/img/zone/'+this.type+'.png');
